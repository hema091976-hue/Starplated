...
import { LayoutDashboard, Settings, QrCode, LogOut, CreditCard, Lock, Menu, X, Users } from 'lucide-react';
...
export function Sidebar({ businessName, isSubscribed, isAdmin }: { businessName: string, isSubscribed: boolean, isAdmin?: boolean }) {
  ...
  const adminNavItems = [
    { name: 'Admin Portal', href: '/dashboard/admin/invite', icon: Users, protected: false },
  ];
    ...
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-[#001d3d] border-b border-white/5 z-30 flex items-center justify-between px-4">
        <div className="flex items-center gap-3 font-bold text-lg">
          ...
          <span className="truncate text-white text-sm uppercase tracking-wide">{isAdmin ? 'Admin' : 'Dashboard'}</span>
        </div>
      ...
      `}>
        <div className="h-20 flex items-center px-6 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-white text-sm font-bold uppercase tracking-wide block">StarPlated</span>
              {isAdmin && <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">Admin</span>}
            </div>
          </div>
        </div>
      
        <div className="p-4 flex-1">
          <div className="text-xs font-bold text-[#fbbc04]/60 uppercase tracking-widest mb-6 px-3">
            {isAdmin ? 'Admin Tools' : 'Main Menu'}
          </div>
        ...
        <div className="p-4 border-t border-white/5">
          <div className="px-3 mb-4">
             ...
             <p className="text-sm font-bold text-white truncate">{isAdmin ? 'StarPlated Admin' : businessName}</p>
          </div>
          <form action={signOut}>
