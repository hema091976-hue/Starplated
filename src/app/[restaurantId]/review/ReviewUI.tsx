...
import { Star, Copy, CheckCircle2, Loader2, MessageSquareQuote, Utensils, Zap, Sparkles, MousePointer2 } from 'lucide-react';
...
export function ReviewUI({ restaurant }: { restaurant: any }) {
  ...
  const handleRating = async (selectedRating: number) => {
    ...
    try {
      ...
      if (response.ok) {
        setOptions(data.options || [{ type: 'Standard', text: data.review, icon: MessageSquareQuote }]);
      }
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const copyAndRedirect = (text: string, index: number) => {
    ...
    setTimeout(() => {
      ...
      // Hide guide after they move away
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] text-[#3c4043] font-sans selection:bg-[#1a73e8]/20 relative overflow-x-hidden">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-6 animate-in fade-in zoom-in duration-300">
          ...
          <div className="relative bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
            ...
            <p className="text-[#70757a] text-sm mb-8">
              We're opening Google now. Just <span className="font-bold text-[#202124]">Paste</span> your review and select <span className="font-bold text-[#202124]">5 stars</span>.
            </p>
            ...
            <div className="space-y-4 mb-2">
              ...
              <div className="flex items-center gap-4 bg-[#f8f9fa] p-4 rounded-2xl border border-[#dadce0]">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[10px] text-[#70757a]">Tap the 5th star</p>
                </div>
                <div className="ml-auto flex gap-0.5">
                  {[1,2,3,4,5].map(i => <Star key={i} size={10} className="fill-[#fbbc04] text-[#fbbc04]" />)}
                </div>
              </div>
            </div>
      ...
      <div className="max-w-xl mx-auto px-6 pt-16 pb-24">
        ...
        {/* Rating Section */}
        <div className="border border-[#dadce0] rounded-2xl p-10 mb-8 text-center bg-[#f8f9fa] shadow-sm">
          ...
          <p className="text-[14px] text-[#70757a]">Tap a star to generate suggested reviews</p>
        </div>
        ...
        {isGenerating && (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-[15px] text-[#70757a] font-medium animate-pulse">Google AI is drafting options...</p>
          </div>
        )}
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between mb-6 px-1">
              ...
              <button 
                ...
                disabled={isGenerating}
                className="text-[11px] font-bold text-[#1a73e8] bg-[#e8f0fe] px-3 py-1 rounded hover:bg-[#d2e3fc] transition-colors flex items-center gap-1.5 uppercase disabled:opacity-50"
              >
                <Sparkles size={12} />
                Regenerate
              </button>
